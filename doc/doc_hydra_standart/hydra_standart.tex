...
\boldblue{(!) Implementation:} Please also note that texture color (that comes from PNG, JPG, BMP and othe LDR formats) should be transformed to linear space with gamma transform: $ C_{linear} = C_{srgb}^{2.2} $.
...
\boldblue{Test:} 101\_diffuse\_lambert.
\begin{figure}[htb]
	\centering
...
\boldblue{Implementation:}
Assume both reflection vector and generated direction (x,y,z) \textbf{are given in local basis} and thus reflection vector equals to (0,1,0). Then, further model can be used for sampling and BRDF evaluation:
...
\boldblue{Test:} 104\_reflect\_phong.
...
\begin{figure}[h]
	...
	\end{minipage}
	\caption{Изображения шарика с моделью фонга для различных степеней косинуса.}
\end{figure}
