\title{Rendering Feature Requrenments}
...
\newcommand{\reqirenment}[1]{%
	...
	\todo[inline, color=green!25]{\textbf{mat\_0.\thesection.\thereq:} \textbf{#1} \hyperref[ruleslist]{(core)}} % Uses the todonotes package to create a fancy box to put the question
...
\newcommand{\reqirenmentextension}[1]{%
	...
	\addcontentsline{def2}{figure}
	{\protect\numberline{\thesection.\thereq}#1}\par
	\todo[inline, color=blue!25]{\textbf{mat\_0.\thesection.\thereq:} \textbf{#1} \hyperref[ruleslist]{(extension)}} % Uses the todonotes package to create a fancy box to put the question
	\vspace{1em} % White space after the question before the start of the answer
}
...
\begin{abstract}
The standard feature number definition consists of 3 parts. They are material, lights and geometry. Thus to determine a finite set of features renderer support you have to specify 3 numbers in subsequent order. For example: <<0.3/0.4/0.1>> means <<0.3>> version for materials, - <<0.4>> for lights and <<0.1>> for geometry. Each number in standart definition means all <<core>> requirenments with less number are are fulfilled. For example, materials ver 0.2 means 0.0.* and 0.1.* requirenemnts are implemented/fulfilled but 0.2.1 may be not implemented/fulfilled.
...
Note that all BRDF and BTDF should be implemented without $cos(\theta_{out})$ term. For mirrors cosine shouldn't be applied, thus you should pre-divide $cos(\theta_{out})$ instead (so that the cosines cancel each other). Also note that we do not include color in BRDF/BTDF equations. The color should be multiplied with the BRDF value.
...
Basic support for Phong reflections. Note that mixing of phong and lambertian reflections is not needed at this level of standard. Renderer may suppose mutually exclusive phong or lambert. 
...
Adding LDR textures for Lambertian materials. When reading from LDR texture, gamma transform \textbf{should} be applied.

\reqirenment{phong\_mirror\_texture\_color}

Adding LDR textures for reflection color for both phong and mirrors. When reading from LDR texture, gamma transform \textbf{should} be applied.
...
Note that all mixtures could be (and in general recommended to be) implemented via blend by bask (see blend\_mask). Howeve, on this level of standart you don't have to implement blend for BRDFs.
