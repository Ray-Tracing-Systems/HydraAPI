...
\begin{abstract}
The standard feature number definition consists of 3 parts. They are material, lights and geometry. Thus to determine a finite set of features renderer support you have to specify 3 numbers in subsequent order. For example: <<0.3/0.4/0.1>> means <<0.3>> version for materials, - <<0.4>> for lights and <<0.1>> for geometry. Each number in standart definition means all <<core>> requirenments with less number are fulfilled. For example, materials ver 0.2 means 0.0.* and 0.1.* requirenemnts are implemented/fulfilled but 0.2.1 may be not implemented/fulfilled.
