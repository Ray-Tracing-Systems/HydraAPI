...
void HRUtils::TransformAllInstances(HRSceneInstRef a_pScn, float a_mat[16])
{
  ...
  if (pScn == nullptr)
  {
    HrError(L"HRUtils::TransformAllInstances: scene is nullptr");
    return;
  }
  ...
  hrSceneOpen(a_pScn, HR_OPEN_READ_ONLY);
  ...
  hrSceneOpen(a_pScn, HR_WRITE_DISCARD);
  ...
  for(auto light : backupListLights)
  {
    ...
    hrLightInstance(a_pScn, tmp, light.m);
  }
