...
struct RD_HydraCore3 : public IHRRenderDriver
{
  RD_HydraCore3();

  ~RD_HydraCore3() override
  {
    ClearAll();
  }

  void GetRenderDriverName(std::wstring &name) override { name = std::wstring(L"HydraCore3");};
...
int RD_HydraCore3::MLT_FrameBufferUpdateLoop()
{
...
void RD_HydraCore3::InitBothDeviceList()
{
#ifdef WIN32
  if (m_clewInitRes == -1)
  {
...
{
  return new RD_HydraCore3;
}


RD_HydraCore3::RD_HydraCore3() : m_pConnection(nullptr), m_pSharedImage(nullptr), m_progressVal(0.0f), m_firstUpdate(true),
...
HRDriverAllocInfo RD_HydraCore3::AllocAll(HRDriverAllocInfo a_info)
{
...
const HRRenderDeviceInfoListElem* RD_HydraCore3::DeviceList() const
{
  if (m_devList2.empty())
    return nullptr;
  else
...
bool RD_HydraCore3::EnableDevice(int32_t id, bool a_enable)
{
  int devNumTotal = 0;
...
void RD_HydraCore3::SetLogDir(const wchar_t* a_logDir, bool a_hideCmd)
{
  ...
  {
      m_pInfoCallBack(L"bad log dir", L"RD_HydraConnectionCore3::SetLogDir", HR_SEVERITY_WARNING);
  }
}
...
bool RD_HydraCore3::UpdateImage(int32_t a_texId, int32_t w, int32_t h, int32_t bpp, int32_t chan, const void* a_data, pugi::xml_node a_texNode)
{
  return true;
}

bool RD_HydraCore3::UpdateImageFromFile(int32_t a_texId, const wchar_t* a_fileName, pugi::xml_node a_texNode)
{
  return true;
}

bool RD_HydraCore3::UpdateMaterial(int32_t a_matId, pugi::xml_node a_materialNode)
{
  return true;
}

bool RD_HydraCore3::UpdateLight(int32_t a_lightIdId, pugi::xml_node a_lightNode)
{
  return true;
}

bool RD_HydraCore3::UpdateMesh(int32_t a_meshId, pugi::xml_node a_meshNode, const HRMeshDriverInput& a_input, const HRBatchInfo* a_batchList, int32_t listSize)
{
  return true;
}

bool RD_HydraCore3::UpdateMeshFromFile(int32_t a_meshId, pugi::xml_node a_meshNode, const wchar_t* a_fileName)
{
  return false;
}

bool RD_HydraCore3::UpdateCamera(pugi::xml_node a_camNode)
{
  return true;
}


bool RD_HydraCore3::UpdateSettings(pugi::xml_node a_settingsNode)
{
...
std::vector<int> RD_HydraCore3::GetCurrDeviceList()
{
  ...
  if (devList.empty())
  {
    ...
    if (m_pInfoCallBack != nullptr)
      m_pInfoCallBack(L"No device was selected; will use device 0 by default", L"RD_HydraConnectionCore3::GetCurrDeviceList()", HR_SEVERITY_WARNING);
  }
...
std::string RD_HydraCore3::MakeRenderCommandLine(const std::string& hydraImageName) const 
{
...
int RD_HydraCore3::GetCurrSharedImageLayersNum()
{
...
void RD_HydraCore3::CreateAndClearSharedImage()
{
  ////////////////////////////////////////////////////////////////////////////////////////////
...
RenderProcessRunParams RD_HydraCore3::GetCurrRunProcessParams()
{
...
void RD_HydraCore3::RunAllHydraHeads()
{
...
void RD_HydraCore3::RunSingleHydraHead(const char* a_cmdLine)
{
  if (m_pConnection == nullptr)
    return;
...
void RD_HydraCore3::BeginScene(pugi::xml_node a_sceneNode)
{
  m_progressVal   = 0.0f;
...
void RD_HydraCore3::EndScene()
{
  
}

void RD_HydraCore3::EndFlush()
{
  if (m_pSharedImage == nullptr)
    return;
  ...
  if(m_enableMLT)
  {
    ...
    m_mltFrameBufferUpdateThread   = std::async(std::launch::async, &RD_HydraCore3::MLT_FrameBufferUpdateLoop, this);
  }
  else
  {
...
void RD_HydraCore3::Draw()
{
  // like glFinish();
}

void RD_HydraCore3::InstanceMeshes(int32_t a_meshId, const float* a_matrices, int32_t a_instNum, const int* a_lightInstId, const int* a_remapId, const int* a_realInstId)
{
  m_instancesNum += a_instNum;
}

void RD_HydraCore3::InstanceLights(int32_t a_light_id, const float* a_matrix, pugi::xml_node* a_custAttrArray, int32_t a_instNum, int32_t a_lightGroupId)
{

}

HRRenderUpdateInfo RD_HydraCore3::HaveUpdateNow(int a_maxRaysPerPixel)
{
  ...
  if (m_instancesNum == 0)
  {
    ...
    result.haveUpdateMSG = false;
    HrPrint(HR_SEVERITY_ERROR, L"RD_HydraConnectionCore3::HaveUpdateNow: no instances in scene!!");
    return result;
  }
...
void RD_HydraCore3::GetFrameBufferLineHDR(int32_t a_xBegin, int32_t a_xEnd, int32_t y, float* a_out, const wchar_t* a_layerName)
{
  if (m_pSharedImage == nullptr)
    return;
...
void RD_HydraCore3::GetFrameBufferLineLDR(int32_t a_xBegin, int32_t a_xEnd, int32_t y, int32_t* a_out)
{
  if (m_pSharedImage == nullptr)
    return;
...
void RD_HydraCore3::LockFrameBufferUpdate()
{
  while(m_colorImageIsLocked); // wait for some fb update thread to set 'm_colorImageIsLocked' to false
  m_colorImageIsLocked = true;
}

void RD_HydraCore3::UnlockFrameBufferUpdate()
{
  m_colorImageIsLocked = false;
}

void RD_HydraCore3::GetFrameBufferHDR(int32_t w, int32_t h, float* a_out, const wchar_t* a_layerName)
{
  const int channels = m_pSharedImage->Header()->channels;
  #pragma omp parallel for
  for (int y = 0; y < h; y++)
...
void RD_HydraCore3::GetFrameBufferLDR(int32_t w, int32_t h, int32_t* a_out)
{
  #pragma omp parallel for
  for (int y = 0; y < h; y++)
...
void RD_HydraCore3::GetGBufferLine(int32_t a_lineNumber, HRGBufferPixel* a_lineData, int32_t a_startX, int32_t a_endX, const std::unordered_set<int32_t>& a_shadowCatchers)
{
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// #TODO: Refactor this
  float* data0 = nullptr;
  float* data1 = nullptr;
...
void RD_HydraCore3::ExecuteCommand(const wchar_t* a_cmd, wchar_t* a_out)
{
