...
float4 main(const SurfaceInfo* sHit, sampler2D texX1, sampler2D texY1, sampler2D texZ1, sampler2D texX2, sampler2D texY2, sampler2D texZ2, float3 tex_scale)
{
  ...
  float2 y_uv = make_float2(pos.x, pos.z);
  float2 x_uv = make_float2(pos.z, pos.y);
  float2 z_uv = make_float2(pos.x, pos.y);
